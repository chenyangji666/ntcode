...
/**
 * Apply env vars from ~/.ntcode/settings.json with highest priority.
 * This ensures ntswitch model selection always wins.
 */
function applyNTCodeOverride(): void {
  try {
    const ntcodePath = join(homedir(), '.ntcode', 'settings.json')
    const raw = readFileSync(ntcodePath, 'utf-8')
    ...
    // ~/.ntcode/settings.json not found or invalid, skip
  }
}
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // ntcode override: ~/.ntcode/settings.json has highest priority
  applyNTCodeOverride()
}
